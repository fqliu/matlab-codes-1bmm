function [ht_es, BIC] = func_1bLR_RELAX_nosigma2(z_bar, X, Nr, Nt, Ns, K, t_bar, N_max, aoa_true, aod_true, H_true)
...
%     tic
   ...
   Ht_pre = H_hat_pre * gamma_hat;
   Ht_ex = H_hat_pre * gamma_hat;
   ...
   while(relative_dif2 > epsilon2)
       ...
       while(relative_dif3 > epsilon3)
           ...
           %% SVT algorithm
           R = Ht_ex - tau * Ht_ex * Xt + tau * (Temp * gamma_hat + Qt); %% Eq. (40)
           [S, V, D] = svd(R);
...
%        iter_num3
       ...
       Tem_d = Ht2 - Ht_hat / gamma_hat;
       relative_dif2 = sum(sum(Tem_d .* conj(Tem_d))) / Nr;
       Ht2 = Ht_hat / gamma_hat;
   end

   
   Ht_hat = Ht_hat / gamma_hat;
...
ht = fft2(H_hat_pre, N_theta, N_varphi);
