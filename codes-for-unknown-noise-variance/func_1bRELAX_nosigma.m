function h = func_1bRELAX_nosigma(z_bar, X, Nr, Nt, Ns, t_bar)
%% assume the scattering path number Ns is known
%%
...
while(s_num<Ns)
    ...
    while(d>1e-5)
        if flag==1
            if es_n==1
                es_vec=NoSigmaNewton(Nr,Nt,X,lambdai,z_bar);
                f5=@(x) (f3(kron(X.'*conj(f2(x(2)).'),f1(x(1)).'))*[x(3);x(4)]-x(5)*lambdai)*sqrt(2);
                fun2=@(x) sum(-log(0.5*erfc(-z_bar.*f5(x)/sqrt(2))));
                [s fmin2]=fmincon(fun2,es_vec,[],[],[],[],[es_vec(1)-W1 es_vec(2)-W2 -inf -inf -inf],[es_vec(1)+W1 es_vec(2)+W2 inf inf inf],[],options);
                At(es_n)=s(1);
                Dt(es_n)=s(2);
                sigma=1/s(5);
                Rt(es_n)=s(3)*sigma;
