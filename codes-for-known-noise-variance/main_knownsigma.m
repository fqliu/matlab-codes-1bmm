...
%% Thresholds vector (real value)/ one can use zero threshods in the known noise variance case
...
h_1bMM_ML = func_1bMM_ML_knownsigma(z_bar, X, Nr, Nt, K, t_bar, Pn);              %% 1bMM-ML

% Low-rank channel estimation
% h_1bMM_LR = func_1bMM_LR_knownsigma(z_bar, X, Nr, Nt, K, t_bar, Pn);              %% 1bMM-LR with orthogonal pilots
h_1bMM_LR = func_1bMM_LR_knownsigma2(z_bar, X, Nr, Nt, K, t_bar, Pn);           %% 1bMM-LR with non-orthogonal pilots

% Angular-domain channel estimation
% [h_1bLR_RELAX, BIC_value] = func_1bLR_RELAX_knownsigma(z_bar, X, Nr, Nt, Ns, K, t_bar, Pn, N_max, aoa/pi*180, aod/pi*180, diag(Hv));  %% 1bLR-RELAX with orthogonal pilots
[h_1bLR_RELAX, BIC_value] = func_1bLR_RELAX_knownsigma2(z_bar, X, Nr, Nt, Ns, K, t_bar, Pn, N_max, aoa/pi*180, aod/pi*180, diag(Hv)); %% 1bLR-RELAX with non-orthogonal pilots

h_1bRELAX = func_1bRELAX_knownsigma(z_bar, X, Pn, Nr, Nt, Ns, t_bar);
...
% Low-rank channel estimation
% h_unqt_LR = func_unqt_LR_knownsigma(Y, X, Nr, Nt, K, Pn);  %% UnqtLR with orthogonal pilots
h_unqt_LR = func_unqt_LR_knownsigma2(Y, X, Nr, Nt, K, Pn);  %% UnqtLR with non-orthogonal pilots
